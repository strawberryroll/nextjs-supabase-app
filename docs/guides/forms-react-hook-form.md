...
> **⚠️ 전제 조건**: 이 프로젝트에는 현재 `react-hook-form`, `@hookform/resolvers`, `zod`가 **설치되어 있지 않다.** 실제 폼(`components/login-form.tsx`, `components/sign-up-form.tsx` 등)은 `useState` + Supabase 클라이언트 직접 호출 패턴을 쓰고 있다. 아래 패턴을 적용하려면 먼저 패키지를 설치해야 한다. 또한 `shadcn/ui`의 `Form`/`FormField` 컴포넌트(`components/ui/form.tsx`)와 `Field`/`FieldGroup` 컴포넌트(`components/ui/field.tsx`)도 아직 설치되어 있지 않으므로 `npx shadcn@latest add form field` 등으로 추가해야 한다.
...
```bash
# 필수 패키지 (설치 필요 — 이 프로젝트에는 아직 없음)
npm install react-hook-form @hookform/resolvers zod

# 고급 기능을 위한 추가 패키지 (선택적)
npm install use-debounce react-error-boundary

# shadcn/ui의 Field 계열 컴포넌트 (아래 최신 폼 예제에 필요)
npx shadcn@latest add field input button card
```
...
### 기본 폼 컴포넌트 패턴 (shadcn/ui 최신 Field 패턴)

> **🔄 shadcn/ui 컴포넌트 변경**: 공식 문서(`ui.shadcn.com/docs/forms/react-hook-form`)의 폼 패턴이 `Form`/`FormField`/`FormItem`/`FormLabel`/`FormMessage` 조합에서 `Field`/`FieldGroup`/`FieldLabel`/`FieldDescription`/`FieldError` + react-hook-form의 `Controller`를 직접 쓰는 방식으로 바뀌었다. 새로 폼을 작성한다면 아래 `Field` 기반 패턴을 사용할 것.
...
### `next/form` + `useActionState` (라이브러리 없는 서버 액션 폼)

React Hook Form 없이 Server Action만으로 폼을 처리할 때는 `next/form`의 `Form` 컴포넌트와 `useActionState`를 조합하는 것이 shadcn/ui 공식 패턴이다. `next/form`은 클라이언트 사이드 네비게이션 프리페치를 지원하는 `<form>` 래퍼다.
...
import { Button } from '@/components/ui/button'
import { Loader2 } from 'lucide-react'
import { bugReportAction, type FormState } from '@/app/actions/bug-report'
...
export function BugReportForm() {
  ...
  return (
    <Form action={formAction}>
      <FieldGroup>
        <Field data-invalid={!!state.errors?.title?.length}>
...
성공 시 서버 액션이 `values`를 생략하고 반환하면 폼이 자동으로 초기화된다:

```typescript
// app/actions/bug-report.ts
'use server'

export async function bugReportAction(_prevState: FormState, formData: FormData) {
  ...
  await saveBugReport(values)

  // ✅ 성공 시 values를 생략하면 React가 폼 상태를 자동 초기화
