...
> **⚠️ 이 프로젝트의 실제 구조와의 차이**: 아래는 규모가 큰 프로젝트를 위한 이상적인 구조 예시로, `src/` 디렉토리 하위에 `app/`, `components/`, `lib/`을 두는 방식이다. 하지만 **이 저장소는 실제로 `src/` 없이 루트에 바로 `app/`, `components/`, `lib/`, `supabase/`를 둔다.** 아래 예시를 적용할 때는 `src/`를 프로젝트 루트(`.`)로 치환해서 읽으면 된다. 분류 규칙, 네이밍 컨벤션, 체크리스트는 구조와 무관하게 그대로 유효하다.
...
> **실제 `components.json` 기준(이 저장소)**: `src/`가 없으므로 별칭은 루트 상대 경로다 — `@/components` → `components`, `@/lib` → `lib`, `@/ui` → `components/ui`, `@/hooks` → `hooks`(아직 폴더 없음). `@/utils`라는 별도 별칭은 정의되어 있지 않으며, `cn()` 등은 `@/lib/utils`로 import한다.
